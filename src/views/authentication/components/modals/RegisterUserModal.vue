...
const submit = async () => {
    if (isFormValid.value) {
        if (user.value) {
            const { year, month, day } = user.value.birthDate;

            if (year && month && day) {
                // Criar um objeto Date (mês no JS começa do 0)
                const birthDate = new Date(year, month - 1, day);

                // Atualizar o birthDate no form (substituir pelo Date)
                user.value.birthDate = birthDate;
            }
            await register(user.value).then(() => {
                close()
                ...
                <div class="flex items-center justify-center min-h-full text-center">
                    <TransitionChild as="div" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel class="w-full overflow-hidden text-left align-middle transition-all transform">
                            <div
                                class="bg-white w-screen h-screen lg:h-auto lg:w-[600px] lg:rounded-[16px] shadow-lg flex flex-col lg:max-h-[90vh]">
                                <!--start header body-->
                                ...
                                <div v-if="!loading"
                                    ...
                                    <RegisterUserForn :is-loading="loading" @formValid="handleFormValid" />
                                </div>
                                <!--end content body-->
