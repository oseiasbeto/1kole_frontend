...
const submit = async () => {
    if (isFormValid.value) {
        if (user.value) {
            ...
            await forgotPassword(user.value.email).then(() => {
                close()
...
<template>
    <TransitionRoot appear :show="modal.show && modal.name == 'forgot password'" as="div">
                        ...
                        <DialogPanel class="w-full overflow-hidden text-left align-middle transition-all transform">
                            <div class="bg-white w-[600px] rounded-[16px] shadow-lg flex flex-col max-h-[90vh]">
                                ...
                                <div v-if="!loading"
                                    ...
                                    <ForgotPasswordForm :is-loading="loading" @formValid="handleFormValid" />
                                </div>
                                <!--end content body-->
