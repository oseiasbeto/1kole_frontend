<template>
    <div>
        <!--start body post-->
        <div>
            <post-list :b-space="112" :posts="replies?.data || []" :is-replies="true" ref="postListComponent"
                ...
                <template #before-content>
                    ...
                    <div v-if="!hasError">
                        <div v-if="!loadingGetPostId">
                            <div ref="originalPostCardRef" class="w-full"
                                v-if="post.is_reply && post?.original_post?._id && !loadingGetOriginalPost">
                                <PostCard :totalItems="0" :post-module="postModule" :index="0" :show-reactions="false"
                                    :is-reply="false" :is-thread="true" :post="post?.original_post" />
                            </div>

                            <div ref="postCardRef">
                                <div class="px-4 h-4 pb-1 w-full">
                                    <div class="w-[44px] flex h-full" v-show="post.is_reply">
                                        <div class="w-0.5 h-full mx-auto bg-light-border dark:bg-dark-border">
                                        </div>
                                    </div>
                                </div>

                                <div class="px-4 pb-[10px] border-b border-light-border dark:border-dark-border"
                                    ...
                                    <div
                                        ...
                                        <author-post-details :is-view-post="true" :show-time="false"
                                            :author="post.author" />
                                        <!--end author details-->
                                    </div>
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            <span
                                                class="text-light-text-primary font-semibold dark:text-dark-text-primary">
                                                {{
                                                    repostsCount }} </span>
                                    ...
                                    <div class="flex justify-between ml-[-3px] items-center gap-5">
                                        <button @click="handleReply(post)"
                                            ...
                                            <span v-show="repliesCount > 0" class="text-inherit text-[15px]">{{
                                                repliesCount
                                                }}</span>
                                        </button>
                                        <button class="flex items-center gap-1 p-[5px] text-[#6f869f] text-sm"
                                            ...
                                            <span v-show="repostsCount > 0" class="text-inherit text-[15px]">{{
                                                repostsCount
                                                }}</span>
                                        </button>
                                        <button class="flex items-center gap-1 p-[5px] text-[#6f869f] text-sm"
                                            ...
                                            <svg v-else fill="none" width="22" viewBox="0 0 24 24" height="22"
                                                class="r-84gixx">
                                                <path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"
...
const isOriginalPostVisible = ref(false);
const originalPostCardRef = ref(null);
    ...
    if (originalPostStore) {
        ...
        if (!post.value?._id) {
            ...
            if (post.value?._id !== route.params.id) {
                ...
                if (post?.value?.is_reply && !post?.value?.original_post._id) {
                    await getOriginalPostById(post?.value?.original_post)
                    console.log("Faz a requesicao para setar o valor do original_post")
                }
                await getReplies({
                    original_post: post.value,
                    page: 1,
