...
const globalVideoState = (() => {
    const activeVideo = ref(null); // Referência ao vídeo ativo
    return {
        getActiveVideo: () => activeVideo.value,
        setActiveVideo: (video) => { activeVideo.value = video; },
        clearActiveVideo: () => { activeVideo.value = null; },
    };
})();
...
const pauseVideo = () => {
    ...
    if (video && !video.paused) {
        ...
        if (globalVideoState.getActiveVideo() === video) {
            globalVideoState.clearActiveVideo(); // Libera o vídeo ativo
        }
    }
};
...
<template>
    <div class="relative w-full max-h-[506px] bg-black">
        ...
        <video ref="videoRef" :poster="props.video.thumbnail || defaultThumb" class="w-full h-full object-cover"
            @click="togglePlay" @dblclick="doubleClickLike" @timeupdate="updateProgress"
            @loadedmetadata="onLoadedMetadata" @ended="onVideoEnded" @waiting="onWaiting" @playing="onPlaying"
        ...
        <div class="absolute bottom-0 w-full px-4 pb-4 flex items-center gap-2">
            ...
            <div class="flex-1 h-1 bg-white bg-opacity-30 cursor-pointer" @click="seekVideo">
                ...
                <div v-else :style="{ width: progress + '%' }"
                    class="h-full bg-white rounded-lg transition-all duration-100"></div>
            </div>
        </div>

        <!-- Botão de mute -->
        <button @click="toggleMute" class="absolute top-4 right-[60px] text-white bg-black bg-opacity-50 rounded-full p-2">
            <svg v-if="isMuted" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="-0.5 0 25 25" fill="none">
                ...
                <path
                    ...
                    stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M22 2.42004L2 22.42" stroke="#fff" stroke-width="1.5" stroke-linecap="round"
